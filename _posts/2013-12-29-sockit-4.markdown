...
with drivers through file IO operations. For reading and writing small bits
...
Device drivers must have a name and a bus. The bus is the way the device the
driver controls is connected to the CPU. This could be PCI, USB, or some other
method. Since our blinker module can be accessed directly from system memory,
we will use the generic [platform](https://www.kernel.org/doc/Documentation/driver-model/platform.txt)
bus type.
...
The mode can be any combination of `S_IWUSR`, meaning the user has a write
...
As we did last time, we will write a byte to memory at address 0xff200000.
However, this address is not yet mapped into the kernel's address space.
The kernel provides functions for properly mapping and accessing the memory
space for peripherals, which is termed IO memory.
...
So now you know how to write a kernel driver. Sort of. There are a lot more
...
to accomplish certain things in your driver.
...
[<- Part 3](/blog/2013/12/27/sockit-3.html)
