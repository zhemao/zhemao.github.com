...
How this works essentially is that the output is the weighted sum of N
...
If you'd like to learn more about how FIR filters work, I've written an
...
this architecture is that the pipeline stages can do useful work at the same
time. That is, on each cycle, data is fetched from the memories, the multipliers
...
`sample_req` is asserted. If we can't do this, the system isn't going to work.
This is the "real-time" in "real-time audio effects".
We are subject to a deadline on how long it takes to respond to an event.
...
That means our computation must take less that about 17 microseconds.
...
because a software prototype (running on my laptop) seemed to give a pretty
good quality at this point.
...
This generates the filter kernel for a windowed sinc filter using a Hamming
window and is normalized so that the DC gain is 1 (i.e. the sum of all the
weights is 1). There is one slight problem here though. These are floating
point numbers, but we are using integer multipliers in our hardware design. 
...
When S is some constant. The results will be the same, except with a little
...
for the for the ROM initialization. I used the
...
what order we go in. If you are trying to use an asymmetric filter (which
...
computation occurring in the later stages of the pipeline. Therefore, we need
...
the output. To get an interesting effect, you can add some delay to the output. 
This will create a sort of echo effect.
...
see how that affects your perception of the sound.
