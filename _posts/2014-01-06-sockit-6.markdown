...
order of the bytes in each word. This is necessary since we will be putting
in the input a word at a time.
...
As you can see `add wave signal` is the basic way of adding a signal to the
